...
- Run `npm run build`.
- Start the app.
- Confirm there is no document-level page scroll.
- Confirm header says `VisualWiki`.
- Confirm tagline says `Your Wikipedia is an image.`
- Confirm memory status shows connected or local memory.
- Generate a page from a prompt.
- Click the generated image and confirm a deeper image is generated.
- Try invalid upload and confirm it is rejected.
- Try export JSON/PDF.
- Confirm no local fallback visual appears.

## Failure Behavior

Image generation failure:

- keep previous image visible
- show retryable error
- do not create a local diagram

Planner failure:

- try JSON repair
- use dynamic topic-derived plan only if repair fails
- still render the final page through the renderer

HydraDB failure:

- continue with empty context
- keep local session state
- log safe status only

## Safe Logging

VisualWiki logs operational events only:

- route started
- planner completed
- image ready
- memory recall/write status
- duration
- session id
- page id
...
## No-Fallback Visual Rule

VisualWiki never generates final pages with:

- local SVG diagrams
- canvas placeholders
- HTML card layouts
- blank local templates
- fake infographic boxes

The final visual page must come from the configured image renderer.

## Deployment Notes

For Vercel or any serverless deployment:

- configure all environment variables in the hosting dashboard
- keep API keys server-side
- use persistent storage if replacing the in-memory MVP store
- keep route timeouts high enough for image generation
- monitor planner and renderer latency
...
VisualWiki is an infinite AI-generated visual browser. Every page is a generated image. Users click anywhere on the image to explore that concept in more depth. HydraDB stores the memory, knowledge, preferences, and exploration graph behind the scenes, so every click becomes part of a persistent visual Wikipedia.
